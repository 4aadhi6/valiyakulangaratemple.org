...
interface AppContextType {
  ...
  logout: () => void;
  templeData: TempleData;
  updateTempleData: (data: Partial<TempleData>) => Promise<void>;
  isLoadingData: boolean;
  fetchTempleData: () => Promise<void>;
  authToken: string | null;
}
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const fetchTempleData = useCallback(async () => {
    setIsLoadingData(true);
    try {
      const data = await templeDataService.getInitialData();
      setTempleData(data);
    } catch (error) {
      console.error("Error fetching temple data:", error);
      setTempleData(defaultTempleData);
    } finally {
      setIsLoadingData(false);
    }
  }, []);
