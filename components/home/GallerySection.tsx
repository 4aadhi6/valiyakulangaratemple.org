    ...
    >
      {templeData.galleryImages.length === 0 ? (
        <p className="text-center text-gray-600 dark:text-gray-400">
          {language === Language.EN
            ? "Gallery images will be updated soon."
            : "ഗാലറി ചിത്രങ്ങള് ഉടൻ അപ്ഡേറ്റ് ചെയ്യുന്നതാണ്"}
        </p>
      ) : (
        <div className="columns-1 sm:columns-2 md:columns-3 lg:columns-4 gap-4 space-y-4">
          {templeData.galleryImages.map((image, index) => (
      ...
      {currentImage && (
        <Modal
          ...
          size="xl"
        >
          <div className="relative max-h-[90vh] overflow-y-auto p-2 sm:p-4">
            <img
              src={currentImage.url}
              alt={
                language === Language.EN
              ...
              className="w-full max-h-[calc(100vh-100px)] object-contain rounded-lg mx-auto"
            />
            <p className="mt-4 text-center text-gray-700 dark:text-gray-300">
              {language === Language.EN
                ? currentImage.description
